...
public class CartManage extends javax.swing.JFrame {
    ...
    try {
        ...
        Logger.getLogger(CartManage.class.getName()).log(Level.SEVERE, "Driver not found", ex);
    } catch (SQLException ex) {
        Logger.getLogger(CartManage.class.getName()).log(Level.SEVERE, "SQL error", ex);
    }
}
    ...
    private void initComponents() {
        ...
        jButtonpay1.setBackground(new java.awt.Color(51, 204, 0));
        jButtonpay1.setFont(new java.awt.Font("Times New Roman", 1, 14)); // NOI18N
        jButtonpay1.setText("Pay");
        jButtonpay1.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jButtonpay1MouseClicked(evt);
            }
        });
        jPanel1.add(jButtonpay1, new org.netbeans.lib.awtextra.AbsoluteConstraints(70, 460, 170, 30));
    ...
    private void jButtonadd10ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonadd10ActionPerformed
        addItemToCart("ChocolateChip Cookie", 1.25, (int) jSpinner10.getValue());
    ...
    private void jButtonpay1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButtonpay1MouseClicked

     DefaultTableModel model = (DefaultTableModel) jTablecartmanage.getModel();
int rowCount = model.getRowCount();

if (rowCount == 0) {
    JOptionPane.showMessageDialog(this, "Cart is empty! Please add items before paying.");
    return;
}

// Get customer name via a popup
String customerName = JOptionPane.showInputDialog(this, "Enter Customer Name:");
if (customerName == null || customerName.trim().isEmpty()) {
    JOptionPane.showMessageDialog(this, "Customer name is required to proceed with the order.");
    return;
}

try {
    // Fetch CustomerID from the database based on the entered CustomerName
    String customerQuery = "SELECT Customer_id FROM customer WHERE Customer_name = ?";
    ...
    if (customerResultSet.next()) {
        ...
        JOptionPane.showMessageDialog(this, "Customer not found in the database. Please register the customer first.");
        return;
    }

    // Insert the order into the database
    String orderQuery = "INSERT INTO orders (CustomerID, CustomerName, OrderDate, DeliveryPartnerNo, Status) VALUES (?, ?, ?, '172157649', 'Ordered')";
    PreparedStatement orderStmt = Con.prepareStatement(orderQuery, PreparedStatement.RETURN_GENERATED_KEYS);

    // Use current date and default status
    java.sql.Date currentDate = new java.sql.Date(System.currentTimeMillis());

    orderStmt.setInt(1, customerId); // Insert the CustomerID
    orderStmt.setString(2, customerName); // Insert the CustomerName
    orderStmt.setDate(3, currentDate);
    ...
    // Insert order details with product_id
    String productQuery = "SELECT product_id FROM products WHERE product_name = ?";
    String orderDetailsQuery = "INSERT INTO order_details (OrderID, ProductID, ProductName, Quantity, Price) VALUES (?, ?, ?, ?, ?)";
    PreparedStatement detailsStmt = Con.prepareStatement(orderDetailsQuery);
    ...
    for (int i = 0; i < rowCount; i++) {
        ...
        if (productResultSet.next()) {
            productId = productResultSet.getInt("product_id");
            System.out.println("Product ID for " + productName + ": " + productId); // Debug log
        } else {
        ...
        // Add order details to the batch
        detailsStmt.setInt(1, orderId);
        detailsStmt.setInt(2, productId);
        detailsStmt.setString(3, productName);
        detailsStmt.setInt(4, quantity);
        detailsStmt.setDouble(5, price);
        detailsStmt.addBatch();

        System.out.println("Order detail added to batch: OrderID = " + orderId + ", ProductID = " + productId +
                ", ProductName = " + productName + ", Quantity = " + quantity + ", Price = " + price); // Debug log
    }

    int[] batchResults = detailsStmt.executeBatch();
    System.out.println("Batch execution results: " + Arrays.toString(batchResults)); // Debug log
    ...
    // Open the OrderTracking JFrame
    new OrderTracking().setVisible(true); // Pass OrderID to OrderTracking
    this.dispose(); // Close the current JFrame if required
    ...
    }//GEN-LAST:event_jButtonpay1MouseClicked
    ...
    try {
        // Iterate through all rows in the cart table
        for (int i = 0; i < jTablecartmanage.getRowCount(); i++) {
            String itemName = jTablecartmanage.getValueAt(i, 0).toString().trim(); // Product name
            int quantity = Integer.parseInt(jTablecartmanage.getValueAt(i, 1).toString()); // Quantity
            double price = Double.parseDouble(jTablecartmanage.getValueAt(i, 2).toString()); // Unit price

            double itemTotal = price * quantity;

            // Debugging: Log the product name being queried
            System.out.println("Fetching discount for product: " + itemName);

            // Fetch discount for the product from the database
            String query = "SELECT discount_value FROM managediscount WHERE product_name = ?";
            PreparedStatement st = Con.prepareStatement(query);
            st.setString(1, itemName);
            ResultSet rs = st.executeQuery();

            double discountValue = 0.0; // Default discount is 0
            if (rs.next()) {
                discountValue = rs.getDouble("discount_value");
                System.out.println("Discount for " + itemName + ": " + discountValue + "%");
            } else {
                System.out.println("No discount found for product: " + itemName);
            }

            double itemDiscount = itemTotal * (discountValue / 100); // Calculate discount
            totalDiscount += itemDiscount;
            total += itemTotal - itemDiscount; // Subtract discount from total
        ...
        JOptionPane.showMessageDialog(this, "Invalid price or quantity format in the cart.");
        e.printStackTrace();
    }
}
